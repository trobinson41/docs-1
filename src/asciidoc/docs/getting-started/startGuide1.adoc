...
+
This action creates a clone of the starter scaffold and opens it in *Project Details*.

* Click *Code Editor* to open the Koji editor.
...
render() {
  return (
    <Container>
      <h1 onClick={this.handleTitleClick}>{this.state.title}</h1>
      <Image src={'https://images.koji-cdn.com/d9c6b38e-08c4-4faf-ae8e-01082c41a0fb/3f83q-9634d620e97345a6b250ca2feb7e5a2e.png'} />
    </Container>
  );
}
...
To test whether the expose function is working, you can use the *Remix* tab of the *Live Preview* pane.
This feature enables you to test the template as if it were published on Koji.

. In the *Live Preview* pane, click *Remix* to switch to the template editing mode.
...
[NOTE]
If the *Remix* is already active, you might need update it by clicking *Default* and then clicking back on *Remix*.
...
. To test the dynamic styling, you can toggle the live preview between the *Remix* and *Default* modes.
* In *Remix* mode, the “editable” styles should be visible, and the click handler should open the VCC, as expected.
* In *Default* mode, the “editable” styles should not be visible, and nothing should happen when clicking the `h1` element.
